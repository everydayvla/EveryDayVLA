...
from prismatic.models.vae import vae_loss_fn
...
class FinetuneConfig:
    ...
    save_freq: int = 20000                          # Checkpoint saving frequency in steps
    ...
    run_id_note: Optional[str] = "DiT"                # Extra note to add to end of run ID for logging
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if use_l1_regression and not use_vae:
            # Predict action
            predicted_actions = action_head.module.predict_noise(actions_hidden_states, 
                ground_truth_actions, batch["attention_mask"].to(device_id))
            # # Get full L1 loss
            # if use_l1_loss:
            #     loss = torch.nn.L1Loss()(ground_truth_actions, predicted_actions)
            # else:
            #     loss_fn = nn.HuberLoss(delta=0.3)
            #     loss = loss_fn(ground_truth_actions, predicted_actions)
            loss = output['loss']
            print(f"loss: ", loss)

        if use_vae:
            predicted_actions = action_head.module.predict_noise(ground_truth_actions, 
            actions_hidden_states, batch["attention_mask"].to(device_id))
            loss = output['loss']
            print(f"loss: ", loss)
        #     # predicted_actions, z, mu, logvar = action_head.module.predict_action(actions_hidden_states)
        #     encoded, predicted_actions, mu, logvar = action_head.module.predict_action(ground_truth_actions, actions_hidden_states)
        #     loss = vae_loss_fn(predicted_actions, ground_truth_actions, mu, logvar)
        ...
        if should_log_l1_loss:
            ...
            # testing
            if use_diffusion_huber_loss:
                print("Using diffusion huber loss!!!")
                loss_fn = nn.HuberLoss(delta=0.5)
                loss = loss + diffusion_huber_loss_weight * loss_fn(ground_truth_actions, predicted_actions)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            compute_diffusion_l1 = cfg.use_vae and batch_idx % cfg.diffusion_sample_freq == 0
