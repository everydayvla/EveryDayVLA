...
# from prismatic.extern.hf.modeling_prismatic_new import OpenVLAForActionPrediction
...
from prismatic.training.train_utils import (
    ...
    compute_l1_loss_between_discrete_continuous_actions,
    compute_alignment_loss,
    compute_alignment_loss_soft_binning,
    EnsembleLoss,
    DeltaActionDecoder
)
...
class FinetuneConfig:
    ...
    vla_path: str = "openvla/openvla-7b-finetuned-libero-object"
    # vla_path: str = "/ix1/rckubendran/sac345/openvla/runs/openvla-7b+six_dof_new_base_dataset+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug"

    # Dataset
    data_root_dir: Path = Path(
        # "/ix1/rckubendran/sac345/datasets/tensorflow_datasets/"
        "/ix1/rckubendran/sac345/modified_libero_rlds/"
        )                                            # Directory containing RLDS datasets
    # dataset_name: str = "six_dof_new_base_dataset"    # Name of fine-tuning dataset (e.g., `aloha_scoop_x_into_bowl`)
    # dataset_name: str = "libero_spatial_no_noops"
    dataset_name: str = "libero_object_no_noops"
    # dataset_name: str = "random_dataset"
    run_root_dir: Path = Path("runs_alt")                # Path to directory to store logs & checkpoints
    shuffle_buffer_size: int = 100_000               # Dataloader shuffle buffer size (can reduce if OOM errors occur)

    # Algorithm and architecture
    use_l1_regression: bool = True                   # If True, trains continuous action head with L1 regression objective
    use_auto_regression: bool = True
    use_ensemble_loss: bool = False
    use_delta_decoder: bool = False
    ...
    # diffusion_sample_freq: int = 1                  # use as an additional loss term
    diffusion_huber_loss_weight = 0.0
    use_diffusion_huber_loss=False
    ...
    # Logging
    wandb_entity: str = "custom_vla"                 # Name of WandB entity
    wandb_project: str = "Custom_VLA_Training"       # Name of WandB project
    run_id_note: Optional[str] = "c2f_proprio_2_cam"                # Extra note to add to end of run ID for logging
...
def run_forward_pass(
    ...
    diffusion_huber_loss_weight=0.5,
    use_diffusion_huber_loss=False,
    use_auto_regression=True,
    ensemble_loss_fn=None,
    delta_decoder_fn=None,
    ...
    regression_loss = 0
    curr_action_l1_loss_reg = 0
    next_actions_l1_loss_reg = 0
    alignment_loss = 0
    delta_loss = 0
    ensemble_loss_dict = {}

    # Compute metrics for discrete action representation (next-token prediction)
    if not (use_l1_regression or use_diffusion) or use_auto_regression:
        loss = output.loss
        ...
        # test_logits = output.logits[:, num_patches:-1]              # [B, T, V]
        # probs = torch.softmax(test_logits, dim=-1)                  # [B, T, V]
        # max_probs, _ = probs.max(dim=-1)                       # [B, T]
        # masked_max_probs = max_probs[current_action_mask | next_actions_mask]  # [N]
        # conf = masked_max_probs.view_as(ground_truth_actions)  # [B, T, D] or [N, D]
        # print("conf.shape: ", conf.shape)
        # print("conf.max: ", conf.max())
        # print("conf.min: ", conf.min())
        # print("conf.mean: ", conf.mean())
        # print("conf: ", conf)
        # lol
        ...
        if use_l1_regression:
            ...
            regression_loss = torch.nn.L1Loss()(ground_truth_actions, predicted_actions)
            ...
            curr_action_l1_loss_reg = torch.nn.L1Loss()(ground_truth_curr_action, predicted_curr_action)
            next_actions_l1_loss_reg = torch.nn.L1Loss()(ground_truth_next_actions, predicted_next_actions)

            # get L1 loss between continuous and discrete action
            # dac_loss = compute_l1_loss_between_discrete_continuous_actions(
            #     action_tokenizer, predicted_token_ids, predicted_actions, 
            #     mask=current_action_mask | next_actions_mask
            # )

            # alignment loss between tokenized continuous actions and discrete actions
            # alignment_loss, action_accuracy = compute_alignment_loss_soft_binning(
            #     action_tokenizer, ground_truth_token_ids, predicted_actions, 
            #     mask=current_action_mask | next_actions_mask
            # )
            # loss += 0.5 * alignment_loss

            if ensemble_loss_fn:
                ar_logits = output.logits[:, num_patches:-1]
                ensemble_loss_dict = ensemble_loss_fn(
                    predicted_actions,
                    predicted_token_ids,
                    ar_logits,                 # Raw logits [B, T, V]
                    ground_truth_actions,
                    ground_truth_token_ids,
                    current_action_mask | next_actions_mask
                )
                loss += ensemble_loss_dict["total_ensemble_loss"].item()

            if delta_decoder_fn:
                delta_loss = delta_decoder_fn(
                    predicted_token_ids, 
                    predicted_actions,
                    ground_truth_actions,
                    ground_truth_token_ids,
        ...
        if ensemble_loss_fn:
            metrics.update(
                {
                    "next_token_curr_action_accuracy": curr_action_accuracy.item(),
                    "next_token_curr_action_l1_loss": curr_action_l1_loss.item(),
                    "next_token_next_actions_accuracy": next_actions_accuracy.item(),
                    "next_token_next_actions_l1_loss": next_actions_l1_loss.item(),
                    # "action_accuracy": action_accuracy.item(),
                    "regression_loss_value": regression_loss.item(),  # Detached value for logging
                    "regression_curr_action_l1_loss": curr_action_l1_loss_reg.item(),
                    "regression_next_actions_l1_loss": next_actions_l1_loss_reg.item(),
                    # "alignment_loss": alignment_loss.item(),
                    "loss_value": loss.item(),
                    "ensemble_l1_loss": ensemble_loss_dict["ensemble_l1"].item(),
                    "soft_clamp_l1_penalty": ensemble_loss_dict["soft_clamp_penalty"].item(),
                    "ensemble_total_loss": ensemble_loss_dict["total_ensemble_loss"].item(),
                }
            )
        elif delta_decoder_fn:
            metrics.update(
                {
                    "next_token_curr_action_accuracy": curr_action_accuracy.item(),
                    "next_token_curr_action_l1_loss": curr_action_l1_loss.item(),
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process and cfg.log_to_wandb:
    ...
    recent_metrics = {
        ...
        "regression_loss_value": deque(maxlen=cfg.grad_accumulation_steps),
        "regression_curr_action_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "regression_next_actions_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "next_token_curr_action_accuracy": deque(maxlen=cfg.grad_accumulation_steps),
        "next_token_curr_action_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "next_token_next_actions_accuracy": deque(maxlen=cfg.grad_accumulation_steps),
        "next_token_next_actions_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        # "action_accuracy": deque(maxlen=cfg.grad_accumulation_steps),
        # "alignment_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "ensemble_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "soft_clamp_l1_penalty": deque(maxlen=cfg.grad_accumulation_steps),
        "ensemble_total_loss": deque(maxlen=cfg.grad_accumulation_steps),
        "delta_loss": deque(maxlen=cfg.grad_accumulation_steps),
    }
    
    if cfg.use_ensemble_loss:
        ensemble_loss_fn = EnsembleLoss(action_tokenizer, ensemble_weight=0.5, clamp_weight=0.1, device_id=device_id)
    else:
        ensemble_loss_fn = None

    if cfg.use_delta_decoder:
        delta_decoder_fn = DeltaActionDecoder(action_tokenizer, device_id=device_id)
    else:
        delta_decoder_fn = None
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss, metrics = run_forward_pass(
                ...
                num_diffusion_steps=cfg.num_diffusion_steps if cfg.use_diffusion else None,
                diffusion_huber_loss_weight=cfg.diffusion_huber_loss_weight,
                use_diffusion_huber_loss=cfg.use_diffusion_huber_loss,
                use_auto_regression=cfg.use_auto_regression,
                ensemble_loss_fn=ensemble_loss_fn,
                delta_decoder_fn=delta_decoder_fn,
            )
            ...
            if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0 and cfg.log_to_wandb:
            ...
            if distributed_state.is_main_process and gradient_step_idx % cfg.wandb_log_freq == 0 and cfg.log_to_wandb:
