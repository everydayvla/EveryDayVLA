...
class FinetuneConfig:
    ...
    num_steps_before_decay: int = 40_000            # Number of steps before LR decays by 10x
    grad_accumulation_steps: int = 1                 # Number of gradient accumulation steps
    max_steps: int = 80005                         # Max number of training steps
    ...
    run_id_note: Optional[str] = "ssm_mixer_diff_L1_steps_5"                # Extra note to add to end of run ID for logging
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if should_log_l1_loss:
            ...
            if use_diffusion_huber_loss:
                # print("Using diffusion L1 loss!!!")
                loss = loss + diffusion_huber_loss_weight * curr_action_l1_loss
...
def run_validation(
    ...
    # return avg_val_metrics["curr_action_l1_loss"]
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                optimizer.step()
                # scheduler.step()
                scheduler.step(normalized_loss)
                optimizer.zero_grad()
                progress.update()
