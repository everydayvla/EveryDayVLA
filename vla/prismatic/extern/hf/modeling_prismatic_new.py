...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _regression_and_discrete_prediction(
        self,
        input_embeddings,
        all_actions_mask,
        projected_patch_embeddings,
        ...
        discretized_actions = self.vocab_size - predicted_action_token_ids
        discretized_actions = np.clip(discretized_actions - 1, a_min=0, a_max=self.bin_centers.shape[0] - 1)
        normalized_actions_discrete = self.bin_centers[discretized_actions]
        normalized_actions_discrete = normalized_actions_discrete.reshape(NUM_ACTIONS_CHUNK, ACTION_DIM)
        ...
        conf = masked_max_probs

        return normalized_actions, normalized_actions_discrete, actions_hidden_states, conf

    
    def predict_hybrid_action(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        unnorm_key: Optional[str] = None,
        proprio=None,
        ...
        **kwargs: str,
        ...
        if use_diffusion:
            ...
            # Run regression and discrete token-based prediction
            normalized_actions, normalized_actions_discrete, actions_hidden_states, confidence = self._regression_and_discrete_prediction(
                    input_embeddings,
                    all_actions_mask,
                    projected_patch_embeddings,
                    attention_mask,
        ...
        # Unnormalize predicted actions
        continuous_actions = self._unnormalize_actions(normalized_actions, unnorm_key)
        discrete_actions = self._unnormalize_actions(normalized_actions_discrete, unnorm_key)

        return continuous_actions, discrete_actions, actions_hidden_states, confidence
