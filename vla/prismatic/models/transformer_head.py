...
class FlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    input_embedding_dim: int = field(
        default=1024, metadata={"help": "Input embedding channel dimension."}
    )
...
class FlowmatchingActionHead(nn.Module):
    ...
    def __init__(
        ...
        # )
        # print("config.action_dim: ", config.action_dim)
        # print("self.input_embedding_dim: ", self.input_embedding_dim)
    ...
    # def forward(self, actions_hidden_states: BatchFeature, action_input: BatchFeature) -> BatchFeature:
    def forward(self, actions_hidden_states: BatchFeature, gt_actions, attn_mask) -> BatchFeature:
        # Set frozen modules to eval
        self.set_frozen_modules_to_eval_mode()

        # Get vision and language embeddings.
        # vl_embeds = backbone_output.backbone_features
        vl_embeds = actions_hidden_states
        device = vl_embeds.device

        # Get embodiment ID.
        # embodiment_id = action_input.embodiment_id
        embodiment_id = embodiment_id = torch.zeros(gt_actions.shape[0], dtype=torch.long, device=gt_actions.device)
    ...
    def get_action(self, backbone_output: BatchFeature) -> BatchFeature:
        ...
        # embodiment_id = action_input.embodiment_id
        embodiment_id = torch.zeros(backbone_output.shape[0], dtype=torch.long, device=backbone_output.device)
...
if __name__ == "__main__":
    ...
    gt_actions = torch.randn(8, 8, 7)
    # action_hidden_states = torch.randn(8, 56, 4096)
    action_hidden_states = torch.randn(8, 56, hidden_dim)
    ...
    model = FlowmatchingActionHead(config=cfg)
    ...
    print(output)
    st = time.time()
    action = model.get_action(action_hidden_states)
    print(f"time for denoising: {time.time() - st}")
