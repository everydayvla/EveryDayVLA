...
from experiments.robot.mad_comp import select_action_by_mad, adapt_horizon, adapt_horizon_with_replan
...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    config["auto_map"] = {
        ...
        "AutoModelForVision2Seq": "modeling_prismatic_new.OpenVLAForActionPrediction",
    }
    ...
    print('  - Set AutoModelForVision2Seq to "modeling_prismatic_new.OpenVLAForActionPrediction"')
...
def get_vla_action(
    ...
    all_images = [obs["full_image"].astype(np.uint8)]
...
def get_hybrid_vla_action(cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
    ...
    if action_head is None:
        ...
        # Custom action head for continuous actions
        continuous_actions, discrete_actions, _, confidence = vla.predict_hybrid_action(
            **inputs,
            unnorm_key=cfg.unnorm_key,
            do_sample=False,
            proprio=proprio,
        ...
        print("Using binning strat")

        confidence = confidence.reshape(continuous_actions.shape[0], continuous_actions.shape[1])

        bin_width = float(action_tokenizer.bins[1] - action_tokenizer.bins[0])
        threshold = bin_width / 2

        # # Compute L1 distance
        # l1_diff = np.abs(continuous_actions - discrete_actions).mean(axis=-1)  # Shape: [B, T]

        # Create a mask: True where continuous is close enough
        # use_continuous = l1_diff <= threshold  # Shape: [B, T]

        # Expand mask to shape [B, T, D] for broadcasting
        # use_continuous = np.expand_dims(use_continuous, axis=-1)

        # Select continuous or discrete per full action vector
        # action = np.where(use_continuous, continuous_actions, discrete_actions)

        print("Using avg strat")
        
        # Step 1: Mean confidence per action vector
        confidence = confidence.cpu().numpy()
        mean_conf = np.mean(confidence, axis=1)
        print("mean_conf:\n", mean_conf)

        # Step 2: High confidence mask
        use_avg_mask = mean_conf >= 0.9

        # Step 3: Expand mask for broadcasting [B, 1]
        use_avg_mask_expanded = np.expand_dims(use_avg_mask, axis=-1)  # shape: [B, 1]

        # Step 4: Compute average of continuous and discrete
        avg_action = (continuous_actions + discrete_actions) / 2

        # Step 5: Blend based on mask
        action = np.where(use_avg_mask_expanded, avg_action, continuous_actions)

    print("continuous_actions: ", continuous_actions)
...
def get_ensembled_vla_action(cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
    ...
    if action_head is None:
        ...
        # action = [ensembler.ensemble_action(discrete_actions[i]) for i in range(len(discrete_actions))]
        # action = ensembler.blend_l1_weighted(continuous_actions, discrete_actions)
        # action = ensembler.conditional_l1_blending(continuous_actions, discrete_actions, confidence)
        # action = discrete_actions
        action = continuous_actions

        # action, mad = ensembler.adapt_horizon_dplan(continuous_actions, 
        #             discrete_actions)
                    
        # thresh = 0.03
        # action = select_action_by_mad(continuous_actions, discrete_actions, threshold=thresh)
        # action = adapt_horizon(continuous_actions, discrete_actions, 
        #                         threshold=0.03, min_actions=6)
        # action = adapt_horizon_with_replan(continuous_actions, discrete_actions, 
        #                         threshold=0.03, replan_threshold=0.06,
        #                         min_actions=4)
...
def get_ensembled_temp_vla_action(cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
    ...
    if action_head is None:
        ...
        print("Using temp ensemble!")
        action = ensembler.temp_ensemble(discrete_actions, timestep)
...
def get_cherried_action(cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
...
def get_ada_vla_action(cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
    ...
    if action_head is None:
        ...
        # action, mad = ensembler.adapt_horizon_dplan(continuous_actions, 
        #             discrete_actions)
        # action = ensembler.switch_actions(continuous_actions, 
        #             discrete_actions)
        # action, mad = ensembler.adapt_horizon_dplan_hist(continuous_actions, 
        #             discrete_actions)
    ...
    # Extract subset of actions for open loop steps
    # return [action[i] for i in range(min(len(action), cfg.num_open_loop_steps))], mad
    return [action[i] for i in range(min(len(action), cfg.num_open_loop_steps))], []